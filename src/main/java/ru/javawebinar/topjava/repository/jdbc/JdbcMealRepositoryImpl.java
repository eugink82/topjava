...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if(meal.isNew()){
            ...
            if(namedParameterJdbcTemplate.update("UPDATE meal SET datetime=:datetime, description=:description," +
                    "calories=:calories WHERE id=:id",mapSqlSource)==0){
                return null;
            };
        }
        return meal;
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        ...
        return namedParameterJdbcTemplate.query("SELECT * FROM meal WHERE user_id=:user_id AND datetime BETWEEN :startDate AND :endDate",mapSqlSource,ROW_MAPPER);
    }
}
